...
public class Robot extends TimedRobot {
  ...
  // public static final DrivetrainSubsystemBase DRIVETRAIN_SUBSYSTEM_BASE = new
  // DrivetrainSubsystemMock();
  ...
  // newly-scheduled
  // commands, running already-scheduled commands, removing fitantnished or
  ...
  private void setNonButtonDependentOverallStates() {
    ...
    // Set the overall state to either scoring alignment or hps pickup based on the
    // zone state
    ...
    // Changes the overall state to empty or loaded transit when the trigger is
    // released
    else if (
    ...
    StateManagement.isManipulatingGamePiece(Robot.overallState, Robot.clawState) == false
        && (Robot.overallState == OverallState.FINAL_SCORING_ALIGNMENT
            || overallState == OverallState.DOUBLE_SUBSTATION_PICKUP)) {
    ...
    // Sets the robot state to PREPARING_TO_SCORE only once when the robot has a
    // piece and is in the alliance community.
    // This conditional is designed as such so it does not continuously set itself
    // when a different overall state such as FINAL_SCORING_ALIGNMENT or SCORING is
    // set.
    ...
    // Same thing but for preparing to collect a game piece when entering the
    // loading zone.
  ...
  private void configureButtonBindings() {
    ...
    // Ground intake.
    GAMEPAD.getButton(ButtonCode.RIGHTBUMPER).and(() -> overallState != OverallState.ENDGAME)
    ...
    // If it was released without a successful ground pickup, state goes back to
    // empty transit
    // If it was released AFTER a successful ground pickup, state goes to loaded
    // transit or preparing to score
    ...
    // Overrides.
    // Drive assist.
    // OP_PAD_SWITCHES.getButton(ButtonCode.ODOMETRY_OVERRIDE).onTrue(new
    // InstantCommand(() -> DRIVER_CONTROL_OVERRIDE = true));
    // OP_PAD_SWITCHES.getButton(ButtonCode.DRIVER_CONTROL_OVERRIDE).onFalse(new
    // InstantCommand(() -> DRIVER_CONTROL_OVERRIDE = false));

    // Vision assist.

    // Old stuff to delete when we're done with the competition code.

    // GAMEPAD.getButton(ButtonCode.A).onTrue(new
    // ArmGoToSetpointDangerousCommand(Constants.ARM_FULL_RETRACT_SETPOINT));
    // GAMEPAD.getButton(ButtonCode.B).onTrue(new
    // ArmGoToSetpointDangerousCommand(Constants.ARM_GROUND_PICKUP_SETPOINT));
    // GAMEPAD.getButton(ButtonCode.X).onTrue(new
    // ArmGoToSetpointDangerousCommand(Constants.ARM_SCORE_CUBE_MID_SETPOINT));
    // GAMEPAD.getButton(ButtonCode.B).onTrue(new
    // ArmGoToSetpointDangerousCommand(Constants.ARM_GROUND_PICKUP_SETPOINT));
    // GAMEPAD.getButton(ButtonCode.LEFTSTICK).onTrue(new
    // ArmGoToSetpointDangerousCommand(Constants.ARM_SCORE_CONE_HIGH_SETPOINT));
    // OP_PAD_SWITCHES.getButton(ButtonCode.RETRACT_ARM).onTrue(new
    // ArmGoToSetpointDangerousCommand(Constants.ARM_SINGLE_SUBSTATION_PICKUP_SETPOINT));

    // OP_PAD_SWITCHES.getButton(ButtonCode.ROTATE_ARM_MAX_ROTATION).onTrue(new
    // ArmGoToSetpointDangerousCommand(Constants.ARM_DOUBLE_SUBSTATION_PICKUP_SETPOINT));//
    // OP_PAD_SWITCHES.getButton(ButtonCode.ROTATE_ARM_TO_ZERO).onTrue(new
    // ArmGoToSetpointDangerousCommand(Constants.ARM_SCORE_LOW_SETPOINT));//

    // OP_PAD_SWITCHES.getButton(ButtonCode.SCORE_LOW).onTrue(new
    // ArmGoToSetpointDangerousCommand(Constants.ARM_SCORE_CONE_MID_SETPOINT));
    // OP_PAD_SWITCHES.getButton(ButtonCode.SCORE_MID).onTrue(new
    // ArmGoToSetpointDangerousCommand(Constants.ARM_SCORE_CUBE_MID_SETPOINT));
    // OP_PAD_SWITCHES.getButton(ButtonCode.SCORE_HIGH).onTrue(new
    // ArmGoToSetpointDangerousCommand(Constants.ARM_SCORE_CONE_HIGH_SETPOINT));
  }
  ...
  private void configureTriggers() {
    ...
    // new Trigger(() -> Robot.CLAW_SUBSYSTEM.detectsGamePiece() && Robot.clawState
    // == ClawState.CLOSED).onTrue(new InstantCommand(() -> Robot.loadState =
    // LoadState.LOADED));

    // If the claw closes but we do NOT have a game piece, pickup failed, so open
    // the claw again.

    // ARM

    // // Extend and rotate the arm to the loading target
    // new Trigger(() -> Robot.loadState == LoadState.EMPTY && (Robot.zoneState ==
    // ZoneState.ALLIANCE_LOADING_ZONE || Robot.overallState ==
    // OverallState.GROUND_PICKUP))
    // .whileTrue(new ArmRotateToRetrievalPositionCommand(Robot.loadTargetState)
    // .andThen(new
    // ArmExtendToRetrievalPositionCommand(Robot.loadTargetState)).withName("Extend
    // and rotate arm to loading target"));

    // // Extend and rotate the arm to the scoring target
    // new Trigger(() -> Robot.loadState == LoadState.LOADED && Robot.zoneState ==
    // ZoneState.ALLIANCE_COMMUNITY)
    // .onTrue(new ArmRotateToRowPositionCommand(Robot.scoringTargetState)
    // .andThen(new
    // ArmExtendToRowPositionCommand(Robot.scoringTargetState)).withName("Extend and
    // rotate arm to scoring target"));
    ...
    // new Trigger(() -> loadState == LoadState.EMPTY &&
    // Robot.CLAW_SUBSYSTEM.detectsGamePiece()).onTrue(RUMBLE_COMMAND);
