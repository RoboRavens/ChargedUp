...
public class ArmGoToSetpointDangerousCommand extends CommandBase {
  ...
  private void calculateSubSetpoints() {
    ...
    if (quadrantBoundariesToCross > 0) {
      for (int i = 0; i < quadrantBoundariesToCross; i++) {
        ...
        double subSetpointRotationNativeUnits = (int) Math.round(subSetpointRotation * Constants.ARM_DEGREES_TO_ENCODER_UNITS);

        ArmMaximumConstraint constraint = ArmPose.calculateMaxExtensionAtAngleDegrees(subSetpointRotation);
        subSetpointExtension = Math.min(finalSetpoint.getExtensionSetpoint(), constraint.getArmLengthToHitConstraintNativeUnits());
        
        ArmSetpoint boundarySubSetpoint = new ArmSetpoint("Boundary SubSetpoint " + i + 1, subSetpointExtension, subSetpointRotationNativeUnits);
  ...
  public void setArmTargets() {
    ...
    if (subSetpoints.size() > setpointIterator) {
      arm.setFinalTargetPositions(finalSetpoint, subSetpoints.get(setpointIterator));
      currentSubSetpoint = subSetpoints.get(setpointIterator);
    }
    else {
      arm.setFinalTargetPositions(finalSetpoint, finalSetpoint);
    }
  }
